...
function operationFromAttentionItem(item: AttentionItemV2, sessionId: string): ProductOperationEnvelope {
  ...
  return {
    ...
    canCancel: false,
    canRetry: item.action.kind === "retry" && item.source === "runtime_operation" && Boolean(item.source_ref?.entity_id),
    currentStep: currentStepFor(status),
    currentStepKey: currentStepKeyFor(status),
    detail,
    id: item.source === "runtime_operation" && item.source_ref?.entity_id ? String(item.source_ref.entity_id) : `attention:${item.key}`,
