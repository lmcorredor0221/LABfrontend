...
export async function generateMetadata({
  ...
  return buildInsightArticleMetadata({
    article,
    language: "pt",
    pathname: buildInsightArticlePath("pt", slug),
    slug,
  });
}
...
export default async function Page({
  ...
  const { slug } = await params;
  const article = findInsightArticle(slug);
  const pathname = buildInsightArticlePath("pt", slug);

  return (
    <>
      <InsightArticleStructuredData article={article} language="pt" pathname={pathname} />
      <InsightArticleDocument language="pt" forceLanguage slug={slug} />
    </>
  );
}
